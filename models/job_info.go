...
type JobInfo struct {
	...
	Salary       float64
	Company      string `orm:"size(128)"`
	Hr           int
	Workplace    string `orm:"size(128)"`
	Years        int
	Education    string `orm:"size(128)"`
	Requirements string `orm:"size(128)"`
	Benefits     string `orm:"size(128)"`
	Datetime     string `orm:"size(128)"`
	ViewCount    uint64
	Interviewee  string `orm:"size(128)"`
}
...
// Id doesn't exist
func GetJobInfoById(id int64) (v *JobInfo, err error) {
	o := orm.NewOrm()
	v = &JobInfo{Id: id}
	if err = o.QueryTable(new(JobInfo)).Filter("Id", id).RelatedSel().One(v); err == nil {
		return v, nil
	}
	return nil, err
}
...
func DeleteJobInfo(id int64) (err error) {
	o := orm.NewOrm()
	v := JobInfo{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
